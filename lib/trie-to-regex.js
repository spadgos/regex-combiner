...
function isSingleToken(token) {
  return token.length === 1 || token[0] === '\\' || tokenIsPositiveCharSet(token);
}
function tokenIsCharSet(token) {
  return token[0] === '[';
}
function tokenIsPositiveCharSet(token) {
  return tokenIsCharSet(token) && token[1] !== '^';
}
...
function stripAndSanitize(token) {
  if (tokenIsCharSet(token)) {
    return token.substring(1, token.length - 1).replace(/^(\^)-|-$/g, '$1\\-');
  }
  if (token === '-') {
    return '\\-';
  }
